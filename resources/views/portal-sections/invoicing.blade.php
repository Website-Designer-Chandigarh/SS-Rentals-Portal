@php
    ...
    $currentMonthInvoices = $this->currentMonthInvoices();
    $currentMonthLabel = $this->currentMonthLabel();
    $allInvoiceTotal = array_sum(array_map(fn($i) => (float) ($i['total'] ?? 0), $currentMonthInvoices));
    $paidTotal = array_sum(array_map(fn($i) => ($i['status'] ?? '') === 'paid' ? (float) ($i['total'] ?? 0) : 0, $currentMonthInvoices));
    $outstandingTotal = array_sum(array_map(fn($i) => ($i['status'] ?? '') !== 'paid' ? (float) ($i['total'] ?? 0) : 0, $currentMonthInvoices));
    $overdueTotal = array_sum(array_map(fn($i) => ($i['status'] ?? '') === 'overdue' ? (float) ($i['total'] ?? 0) : 0, $currentMonthInvoices));
...
<div>
    ...
    @if($invoiceTab === 'overdue' && $overdueTotal > 0)
        <div class="alert alert-red mb-4">
            <div><strong>{{ count(array_filter($currentMonthInvoices, fn($i) => ($i['status'] ?? '') === 'overdue')) }} invoices overdue</strong> — total overdue amount is {{ $this->money($overdueTotal) }}. Follow up directly with overdue customers.</div>
        </div>
    @endif
